<script>
  ...
  let reworkActive = $derived(qsState.reworksOf && filteredList.length > 1)
  ...
  function onClick(event) {
    ...
    if (caller.classList.contains('work--composition-date')) {
      onRefine(serialize('d', facets.d))
    }

    if (caller.classList.contains('work--duration')) {
      onRefine(serialize('t', facets.t))
    }
    ...
    if (caller.classList.contains('category')) {
      onRefine(serialize('c', facets.c))
    }

    if (caller.classList.contains('work--language')) {
      onRefine(serialize('l', facets.l))
    }
    ...
    if (caller.classList.contains('work--detail-toogle')) {
      ...
      if (fieldsEl) {
        ...
        mount(WorkFields, {
          target: workEl,
          anchor: caller, // Fonctionne si caller est un nœud DOM dans workEl
          props: {
            fields,
            i18n,
            publishers,
