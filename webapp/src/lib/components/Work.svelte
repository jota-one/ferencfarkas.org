<script lang="ts">
  import type { Category, I18n, RenderedWork } from '../../types'
  ...
  <header>
    <div class="work--meta">
      ...
      <button class="work--duration">
        {work.duration ? formatHelper.formatDuration(work.duration) : '-'}
      </button>
    ...
    <div class="work--multimedia">
      ...
      {#if work.scores}
        ...
        <div class="work--scores">
          <button onclick={e => {
            (e.target as HTMLButtonElement).parentElement?.classList.toggle('hover')}
          }>
            Scores
